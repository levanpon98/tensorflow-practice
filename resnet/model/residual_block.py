...
def make_basic_block_layer(filters, blocks, stride=1):
    res_block = tf.keras.Sequential()
    res_block.add(BasicBlock(filters, strides=stride))
...
def make_bottleneck_layer(filters, blocks, stride=1):
    res_block = tf.keras.Sequential()
    res_block.add(BottleNeck(filters, strides=stride))

    for _ in range(1, blocks):
        res_block.add(BottleNeck(filters, strides=1))
